...
# (2) htf_bot author partner
bot_partner_id = False
try:
    bot_user = env.ref('htf_call_center.user_htf_bot', raise_if_not_found=False)
    if bot_user:
        bot_partner_id = bot_user.partner_id.id
except Exception:
    pass
if not bot_partner_id:
    bot_user = User.search([('login', '=', 'htf_bot')], limit=1)
    if bot_user:
        bot_partner_id = bot_user.partner_id.id
print('BOT_PARTNER_FOUND:' + ('1' if bot_partner_id else '0'))
...
ids_by_author = set()
if bot_partner_id:
    ids_by_author = set(Msg.search([('author_id', '=', bot_partner_id)]).ids)
print('IDS_BY_AUTHOR:' + str(len(ids_by_author)))
...
all_ids = ids_by_subtype | ids_by_author | ids_by_channel
...
def survey(db: str) -> dict[str, int]:
    ...
    log(f'  htf_bot partner resolved : {bool(stats["bot_partner_found"])}')
    log(f'  mirrored channels        : {stats["mirrored_channels"]}')
    log(f'  msgs matched by subtype  : {stats["by_subtype"]}')
    log(f'  msgs matched by author   : {stats["by_author"]}')
